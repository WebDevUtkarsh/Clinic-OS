...
import type { FinancialTrendPoint } from "@/lib/backend/reports/types";
...
export async function GET(req: NextRequest) {
  ...
  let data = await getCachedReport<FinancialTrendPoint[]>(cacheKey);

  if (!data) {
    const prisma = await getTenantPrisma(tenantId);
    data = await getFinancialTrendsReport(prisma, {
      tenantId,
