...
import type { PaymentMethodReport } from "@/lib/backend/reports/types";
...
export async function GET(req: NextRequest) {
  ...
  let data = await getCachedReport<PaymentMethodReport>(cacheKey);

  if (!data) {
    const prisma = await getTenantPrisma(tenantId);
    data = await getPaymentMethodReport(prisma, {
