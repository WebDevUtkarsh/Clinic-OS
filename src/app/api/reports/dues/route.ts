...
import type { OutstandingDuesReport } from "@/lib/backend/reports/types";
...
export async function GET(req: NextRequest) {
  ...
  let data = await getCachedReport<OutstandingDuesReport>(cacheKey);

  if (!data) {
    const prisma = await getTenantPrisma(tenantId);
    data = await getOutstandingDuesReport(prisma, {
